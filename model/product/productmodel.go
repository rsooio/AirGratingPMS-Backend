...
var (
	cacheProductProductSetListCount = "cache:product:product_set:list:count"
...
func (m *customProductModel) Insert(ctx context.Context, data *Product) (sql.Result, error) {
	m.DelCacheCtx(ctx, cacheProductProductSetListCount)
	return m.defaultProductModel.Insert(ctx, data)
}

func (m *customProductModel) Delete(ctx context.Context, id int64) error {
	m.DelCacheCtx(ctx, cacheProductProductSetListCount)
	return m.defaultProductModel.Delete(ctx, id)
}
...
func (m *customProductModel) FindListByProductSetId(ctx context.Context, productSetId int64, offset, limit int32) (ProductList, int64, error) {
	...
	err := mr.Finish(func() error {
		...
		if err != nil {
			logx.Errorf("productProductSetId.findList error, orderId=%d, offset=%d, limit=%d, err=%s", productSetId, offset, limit, err.Error())
		...
		return m.QueryRow(&count, cacheProductProductSetListCount, func(conn sqlx.SqlConn, v interface{}) error {
